...
class Mesh:
    ...
    def _triangulate(self):
        ...
        Stores:
            ...
            self.tri = triangulation of the vertices (nv x 3) matrix. i.e. The centroid triangles that correspond to each vertex.
        ...
        #   and the rest are translations. x is the position of each centroid in the "true" frame. 
        #   y is the position of each centroid in the true and 8 additional imaged frames
    ...
    def get_l_interface(self):
        """
        Constructs a sparse matrix representing the connectivity between the vertices of the voronoi cells in the mesh.

        This method computes a COO sparse matrix where each value corresponds to the interface length between two vertices,
        as defined by the triangulation and its periodic roll. This constitutes an effective adjacency matrix weighted by 
        the interface lengths.

        :return: None. Updates self.l_int with the interfacial length matrix.
        """

...
def tnorm(x):
    """
    Triangle Norm - Calculate the L1 norm of a set of vectors that are given in triangulated form:
...
def circumcenter(C, L):
    ...
    """
    ri, rj, rk = C.transpose(1, 2, 0)  # gives positions of centroids i,j,k in each triangle
    r_mean = (ri + rj + rk) / 3
    disp = r_mean - L / 2
    ri, rj, rk = np.mod(ri - disp, L), np.mod(rj - disp, L), np.mod(rk - disp, L)  # apply periodic BCs
    ax, ay = ri  # x and y coords of centroid i in each triangl, etc.
    bx, by = rj
    cx, cy = rk
    d = 2 * (ax * (by - cy) + bx * (cy - ay) + cx * (ay - by))  # twice square of m0agntidue of cross product of (ri-rj) and (rj-rk), following Bi et al. 2016
    ux = ((ax * ax + ay * ay) * (by - cy) + (bx * bx + by * by) * (cy - ay) + (cx * cx + cy * cy) * (
            ay - by)) / d  # x coord of circumcentre (i.e. vertex of voronoi cell)
    uy = ((ax * ax + ay * ay) * (cx - bx) + (bx * bx + by * by) * (ax - cx) + (cx * cx + cy * cy) * (
            bx - ax)) / d  # y coord of circumcentre (i.e. vertex of voronoi cell)
    vs = np.empty((ax.size, 2), dtype=np.float64)
    vs[:, 0], vs[:, 1] = ux, uy
    vs = np.mod(vs + disp.T, L)  # complete list of all vertices
    return vs

...
def animate(x_save, phi_save, L, dt, tskip, cols, n_frames=100, file_name=None, dir_name="plots", cbar=None, **kwargs):
    """
    Animate the simulation

    :param x_save: nt x nc x 2 array of positions
    :param phi_save: nt x 3 array of boundary fractions and boundary counts
    :param L: domain size
    :param cols: either a nc array of strings defining fixed colors, or a nt x nc array if colors are to vary.
    :param n_frames: number of frames to plot
    ...
    frame_skip = int((x_save.shape[0]) / n_frames)

    time_points = np.arange(len(phi_save)) * dt * tskip  # getting physical time points at each saved snapshot
    phi_vals = phi_save[:, 2] if phi_save.ndim > 1 else phi_save
    ...
    ax2.set_ylim(0, 1)
    ax2.set_xlabel("Time")
    ax2.set_ylabel(r"Boundary fraction ($\phi_b$)")
    ...
    def animate(i):
        ...
        if dynamic_col:
            plot_vor(ax1, x_save[frame_skip * i], L, cols[frame_skip * i], cbar=cbar_a, **kwargs)
        else:
            plot_vor(ax1, x_save[frame_skip * i], L, cols, cbar=cbar_a, **kwargs)
        
        # Display the boundary fraction
        phi_val = phi_save[frame_skip * i, 2]
        ax1.text(0.02 * L, 0.95 * L, r"$\phi_b$: " + f"{phi_val:.3f}",
        ...
        # Update φ(t) line
        line_phi.set_data(time_points[: frame_skip * i + 1], phi_vals[: frame_skip * i + 1])
        ax2.set_xlim(time_points[0], time_points[-1])  # keep static
        return []
